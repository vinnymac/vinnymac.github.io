...
this.collection.each((m) => {
  if (m.get('state') === 'accepted') this.accepted.push(m);
});
```
...
member.size; // → 2
for (let [key, val] of member.entries()) console.log(key + ' = ' + val);
```
